        ...
        try {
            ...
            try (Connection con = DatabaseConnection.getConnection()) {
                try (PreparedStatement ps = con
                        .prepareStatement("SELECT MAX(uniqueid) FROM inventoryitems WHERE uniqueid > 0");
                ...
                try (PreparedStatement ps = con.prepareStatement("SELECT MAX(petid) FROM pets");
                        ResultSet rs = ps.executeQuery()) {
                    if (rs.next()) {
                        ids[1] = rs.getInt(1) + 1;
                    }
                ...
                try (PreparedStatement ps = con.prepareStatement("SELECT MAX(ringid) FROM rings");
                        ResultSet rs = ps.executeQuery()) {
                    if (rs.next()) {
                        ids[2] = rs.getInt(1) + 1;
                    }
                ...
                try (PreparedStatement ps = con.prepareStatement("SELECT MAX(partnerringid) FROM rings");
                        ResultSet rs = ps.executeQuery()) {
                    if (rs.next()) {
                ...
                try (PreparedStatement ps = con.prepareStatement("SELECT MAX(uniqueid) FROM androids");
                        ResultSet rs = ps.executeQuery()) {
                    if (rs.next()) {
                        ids[4] = rs.getInt(1) + 1;
                    }
