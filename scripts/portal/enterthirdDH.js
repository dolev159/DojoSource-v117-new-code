...
function enter(pi) {
	...
	if (pi.getMap(913010200).getCharacters().size() > 0) {
		...
		pi.getClient().getSession().write(Packages.tools.packet.MaplePacketCreator.serverNotice(5, "The only way you may enter the 3rd Hall is if you're training for the Level 100 skills."));
		return false;
}