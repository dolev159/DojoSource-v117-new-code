...
function enter(pi) {
	...
	if (pi.getMap(910400000).getCharacters().size() < 1) {
		...
		pi.getClient().getSession().write(Packages.tools.packet.MaplePacketCreator.serverNotice(5, "The door to the info shop is not opening. Something must have happened! Try again in a bit."));
		return false;
		}
