...
function enter(pi) {
	...
	if (pi.getPlayer().getMap().getId() == 809061000 && level < 120) {
		pi.getClient().getSession().write(Packages.tools.packet.MaplePacketCreator.serverNotice(5, "At least level 120 or above is required to enter."));
		return false;
		}
	if (pi.getPlayer().itemQuantity(4001433) < 20) {
		pi.getClient().getSession().write(Packages.tools.packet.MaplePacketCreator.serverNotice(5, "To worship the Six Hand Evil God Temple, it is necessary to worship 20 sparks of the sun."));
		return false;
		}
