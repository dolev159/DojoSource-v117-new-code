...
function enter(pi) {
	...
	if (eim.getProperty(i + "st") == x) {
		pi.getClient().getSession().write(Packages.tools.packet.EtcPacket.instantMapWarp(pi.getPlayer().getMap().getPortal("np0" + pi.getPortal().getName().substring(2, 3)).getId()));
