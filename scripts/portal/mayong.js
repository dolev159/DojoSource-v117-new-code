...
function enter(pi) {
	...
	if (pi.getMap(924000200).getCharacters().size() > 0) {
		pi.getClient().getSession().write(Packages.tools.packet.MaplePacketCreator.serverNotice(5, "There is an another adventurer being tested inside... Move to a different channel so you don't disturb them."));
		return false;
		}
