...
function enter(pi) {
	...
	if (pi.getMap(931010030).getCharacters().size() < 1) {
		...
		pi.getClient().getSession().write(Packages.tools.packet.MaplePacketCreator.serverNotice(5, "Question Fabio about what he's been doing!"));
		return true;
		}
		pi.getClient().getSession().write(Packages.tools.packet.MaplePacketCreator.serverNotice(5, "The Hair Salon seems to be full of customers. Try coming back later."));
		return false;
		}
