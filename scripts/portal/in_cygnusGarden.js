...
function enter(pi) {
	if (pi.getPlayer().itemQuantity(4032923)) {
		pi.getClient().getSession().write(Packages.tools.packet.MaplePacketCreator.serverNotice(5, "Entering Cygnus Garden."));
	...
	if (pi.getPlayer().getQuestNAdd(Packages.server.quest.MapleQuest.getInstance(31149)).getStatus() == 1) {
		...
		pi.getClient().getSession().write(Packages.tools.packet.MaplePacketCreator.serverNotice(5, "Confirmed the location of Cygnus Garden."));
		return false;
		}
