...
function enter(pi) {
	...
	if (gate != null && gate.getState() == 4) {
		...
		pi.getClient().getSession().write(Packages.tools.packet.CField.instantMapWarp(pi.getPlayer().getMap().getPortal("gt" + name + "PIA").getId()));
		...
		pi.getClient().getSession().write(Packages.tools.packet.CWvsContext.serverNotice(6, "The door is not opened yet."));
		return false;
}