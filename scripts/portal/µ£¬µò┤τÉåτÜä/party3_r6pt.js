...
function enter(pi) {
	...
	if (eim.getProperty(i + "st") == x) {
		pi.getClient().getSession().write(Packages.tools.packet.CField.instantMapWarp(pi.getPlayer().getMap().getPortal("np0" + (pi.getPortal().getName().substring(3, 4) - 1)).getId()));
		...
		pi.getClient().getSession().write(Packages.tools.packet.CField.instantMapWarp(pi.getPlayer().getMap().getPortal(x.substring(2, 5) > 54 ? "np03" : "np16").getId()));
