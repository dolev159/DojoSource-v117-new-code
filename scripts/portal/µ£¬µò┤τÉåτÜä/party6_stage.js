...
function enter(pi) {
	...
	if (pi.getPlayer().getMap().getAllMonstersThreadsafe().size() < 1) {
		...
		pi.getClient().getSession().write(Packages.tools.packet.CWvsContext.serverNotice(6, "Eliminate all the monsters."));
		return false;
		}
	if (pi.getPlayer().getMap().getId() == 930000200) {
	if (pi.getPlayer().getMap().getReactorByName("spine").getState() < 4) {
		pi.getClient().getSession().write(Packages.tools.packet.CWvsContext.serverNotice(6, "The spine blocks the way."));
		return false;
		}
