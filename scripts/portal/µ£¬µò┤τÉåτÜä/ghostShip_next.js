...
function enter(pi) {
	if (pi.getPlayer().getMap().getAllMonstersThreadsafe().size() != 0) {
		pi.getClient().getSession().write(Packages.tools.packet.CWvsContext.serverNotice(6, "Please, clear all of the monsters!"));
		return false;
		}
	if (pi.getPlayer().getParty().getLeader().getId() != pi.getPlayer().getId()) {
		pi.getClient().getSession().write(Packages.tools.packet.CWvsContext.serverNotice(6, "The leader must be here."));
		return false;
		}
	if (((pi.getPlayer().getMap().getId() % 10) | 0) == 4) {
		...
		pi.getClient().getSession().write(Packages.tools.packet.CWvsContext.serverNotice(6, "Both switches must be turned on."));
		return false;
		}
