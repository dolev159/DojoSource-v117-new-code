...
function enter(pi) {
	if (pi.getPlayer().getQuestNAdd(Packages.server.quest.MapleQuest.getInstance(3368)).getStatus() != 1) {
		pi.getClient().getSession().write(Packages.tools.packet.CWvsContext.serverNotice(6, "You cannot access this area."));
		return false;
		}
	if (pi.getMap(926130103).getCharacters().size() > 0) {
		pi.getClient().getSession().write(Packages.tools.packet.CWvsContext.serverNotice(6, "Someone is already in this map, Better come back later."));
		return false;
		}
