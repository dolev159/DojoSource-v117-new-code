...
function action0(mode, type, selection) {
	...
	case 1:
		...
		break;
	case 2:
		if (cm.getPlayer().itemQuantity(5152010)) {
			cm.gainItem(5152010, -1);
			cm.getPlayer().setFace(color);
			cm.getPlayer().updateSingleStat(Packages.client.MapleStat.FACE, color);
			cm.sendNext("Tada~! Check it out!! What do you think? I really think your eyes look sooo fantastic now~~! Please come again ~");
			cm.dispose();
			return;
			}
...
function action1(mode, type, selection) {
	...
	case 1:
		...
		break;
	case 2:
		if (cm.getPlayer().itemQuantity(5152013)) {
