...
function action(mode, type, selection) {
	...
	case 0:
		...
		if (cm.getPlayer().getQuestNAdd(Packages.server.quest.MapleQuest.getInstance(28767)).getStatus() > 0) {
			cm.sendOkS("Mess up their computers.", 3);
			cm.dispose();
			return;
			}
		if (cm.getPlayer().getPosition().x < 1100 || cm.getPlayer().getPosition().x > 1400 || cm.getPlayer().getPosition().y < -700) {
			...
			break;
	case 1:
		cm.sendNextS("(Hey, this has some information on Masteria. Maybe I should take a little peek...)", 3);
		break;
	case 2:
		cm.getClient().getSession().write(Packages.tools.packet.EtcPacket.UIPacket.getDirectionInfo(1, 500));
		break;
	case 3:
		cm.getClient().getSession().write(Packages.tools.packet.MaplePacketCreator.getTopMsg("Sabotage (Information Liberation) complete"));
	...
	case 12:
		...
		cm.getClient().getSession().write(Packages.tools.packet.EtcPacket.UIPacket.getDirectionInfo(1, 200));
		break;
	case 13:
		cm.getClient().getSession().write(Packages.tools.packet.EtcPacket.NPCPacket.removeNPCController(9201174));
		cm.spawnNPCRequestController(9201174, 1133, -611, 1);
	...
	case 15:
		...
		break;
	case 16:
		cm.sendNextS("My account got hacked! I knew I should've put a stronger password on this thing!", 5, 9201174);
		break;
	case 17:
		cm.getClient().getSession().write(Packages.tools.packet.EtcPacket.NPCPacket.removeNPCController(9201174));
		cm.getClient().getSession().write(Packages.tools.packet.EtcPacket.UIPacket.getDirectionInfo(1, 1000));
	...
	case 18:
		...
		break;
	case 19:
		cm.sendNextS("That was an entirely preventable hack-job!", 3);
		break;
	case 20:
