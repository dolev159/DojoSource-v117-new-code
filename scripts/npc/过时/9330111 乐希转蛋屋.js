    ...
    if (status == 0) {
        ...
        sel = selection;
		 ...
		 if (select >= 0 && select <= 9) {
			 cm.sendYesNo("您确定要使用#v"+itemchange+"#" + items[select][sel][1] + "个兑换#v"+items[select][sel][0]+"#?");
        }
...
function chooseItem(index) {
    ...
    for (var i = 0; i < items[index].length; i++)
        choice += "\r\n#L" + i + "# #v"+itemchange+"#" + items[index][i][1] + "个交换 #v" + items[index][i][0] + "#(#z" + items[index][i][0] + "#)" + (items[index][i][2] > 0 ? ("#r#e" + items[index][i][2] + "#n#b个") : "") + (items[index][i][3] > 0 ? (" 期限#r#e" + items[index][i][3] + "#n#b分钟") :"") + "#l";
...
function gainReward(index) {
    ...
    if (itemQuantity >= items[index][sel][1]) {
        ...
        cm.sendOk("请确认#v"+itemchange+"#是否足够");
    }
}
