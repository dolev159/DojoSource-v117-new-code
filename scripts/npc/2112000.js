...
function action(mode, type, selection) {
	...
	case 1:
		...
		reactor = 'action' + (cm.getPlayer().getMap().getId() == 926100203 ? 0 : cm.getMap(926100401).getReactorById(2619001).getState() < 1 ? 1 : 2);
...
function action1(mode, type, selection) {
	...
	case 1:
		if (cm.getPlayer().getParty().getLeader().getId() != cm.getPlayer().getId()) {
			...
			break;
	case 2:
		cm.removeNpc(cm.getPlayer().getMap().getId(), 2112000);
		cm.getPlayer().getMap().startMapEffect("Please protect Juliet by defe ating Frankenroid!", 5120021);
		cm.getPlayer().getMap().spawnMonsterOnGroundBelow(Packages.server.life.MapleLifeFactory.getMonster(9300139), new java.awt.Point(250, 100));
...
function action4(mode, type, selection) {
	...
	case 3:
		cm.removeNpc(cm.getPlayer().getMap().getId(), 2112000);
		cm.getPlayer().getMap().startMapEffect("Please protect Juliet by defe ating Frankenroid!", 5120021);
		cm.getPlayer().getMap().spawnMonsterOnGroundBelow(Packages.server.life.MapleLifeFactory.getMonster(9300140), new java.awt.Point(250, 100));
