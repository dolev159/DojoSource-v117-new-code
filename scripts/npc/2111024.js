...
function action(mode, type, selection) {
	if (mode > 0) {
		...
		if (cm.getPlayer().getInfoQuest(3360).indexOf("arr2=o") != -1 || cm.getPlayer().getInfoQuest(3360).indexOf("arr3=o") != -1) {
			...
			cm.getClient().getSession().write(Packages.tools.packet.CWvsContext.serverNotice(5, "The security device has been unlocked. Your name has entered the list."));
			cm.getClient().getSession().write(Packages.tools.packet.CWvsContext.getShowQuestCompletion(3360));
			...
			cm.getClient().getSession().write(Packages.tools.packet.CWvsContext.serverNotice(5, "The security device has been unlocked."));
			cm.dispose();
			return;
			}
			cm.getClient().getSession().write(Packages.tools.packet.CWvsContext.serverNotice(5, "Incorrect passcode. You are not authorized to enter the Secret Passage."));
			}
			cm.dispose();
}