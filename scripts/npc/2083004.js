...
function start() {
    ...
    if (cm.getPlayer().getClient().getChannel() != 1 && cm.getPlayer().getClient().getChannel() != 3) {
        cm.sendOk("Horntail may only be attempted on channel 1 and 3");
        cm.dispose();
        return;
    }
    var em = cm.getEventManager("gmmap");
    ...
    if (prop == null || prop.equals("0")) {
        var squadAvailability = cm.getSquadAvailability("gmmap");
        if (squadAvailability == -1) {
            ...
            var type = cm.isSquadLeader("gmmap");
            if (type == -1) {
                cm.sendOk("The squad has ended, please re-register.");
                cm.dispose();
            } else if (type == 0) {
                var memberType = cm.isSquadMember("gmmap");
                if (memberType == 2) {
            ...
            var eim = cm.getDisconnected("gmmap");
            if (eim == null) {
                var squd = cm.getSquad("gmmap");
                if (squd != null) {
                    if (time + (12 * 3600000) >= cm.getCurrentTime() && !cm.getPlayer().isGM()) {
                        cm.sendOk("You have already went to Horntail in the past 12 hours. Time left: " + cm.getReadableMillis(cm.getCurrentTime(), time + (12 * 3600000)));
                        cm.dispose();
...
function action(mode, type, selection) {
    ...
    switch (status) {
        case 0:
            if (mode == 1) {
                if (cm.registerSquad("gmmap", 5, " has been named the Leader of the squad (Regular). If you would you like to join please register for the Expedition Squad within the time period.")) {
        ...
        case 5:
            if (selection == 0) {
                if (!cm.getSquadList("gmmap", 0)) {
                    cm.sendOk("Due to an unknown error, the request for squad has been denied.");
                }
            } else if (selection == 1) { // join
                var ba = cm.addMember("gmmap", true);
                if (ba == 2) {
                ...
                var baa = cm.addMember("gmmap", false);
                if (baa == 1) {
        ...
        case 10:
            if (mode == 1) {
                if (selection == 0) {
                    ...
                    status = 11;
                    if (!cm.getSquadList("gmmap", 1)) {
                        cm.sendOk("Due to an unknown error, the request for squad has been denied.");
                        cm.dispose();
                    }
                } else if (selection == 2) {
                    status = 12;
                    if (!cm.getSquadList("gmmap", 2)) {
                        cm.sendOk("Due to an unknown error, the request for squad has been denied.");
                        cm.dispose();
                    }
                } else if (selection == 3) { // get insode
                    if (cm.getSquad("gmmap") != null) {
                        var dd = cm.getEventManager("gmmap");
                        dd.startInstance(cm.getSquad("gmmap"), cm.getMap(), 160100);
                    } else {
                        cm.sendOk("Due to an unknown error, the request for squad has been denied.");
                    }
                    cm.dispose();
            ...
            break;
        case 11:
            cm.banMember("gmmap", selection);
            cm.dispose();
            break;
        case 12:
            if (selection != -1) {
                cm.acceptMember("gmmap", selection);
            }
            cm.dispose();
            break;
        default:
