...
function action(mode, type, selection) {
	...
	case 1:
		...
		reactor = 'action' + (cm.getPlayer().getQuestNAdd(Packages.server.quest.MapleQuest.getInstance(4900)).getStatus() < 1 ? 0 : 1);
...
function action0(mode, type, selection) {
	...
	case 1:
		...
		if (selection == 10) {
			Packages.server.quest.MapleQuest.getInstance(4900).forceStart(cm.getPlayer(), cm.getNpc(), 0);
...
function action1(mode, type, selection) {
	switch (status) {
	case 0:
		if (cm.getPlayer().getQuestNAdd(Packages.server.quest.MapleQuest.getInstance(4911)).getStatus() > 1) {
