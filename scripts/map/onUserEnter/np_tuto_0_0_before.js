...
function action(mode, type, selection) {
	...
	case 0:
		...
		break;
	case 1:
		ms.getMap().broadcastMessage(Packages.tools.packet.CField.environmentChange("newPirate/text0", 3));
		ms.getClient().getSession().write(Packages.tools.packet.CField.UIPacket.getDirectionInfo(1, 11000));
		break;
	case 2:
		ms.dispose();
		ms.getPlayer().changeMap(ms.getMap(552000010), ms.getMap(552000010).getPortal(0));
}
}