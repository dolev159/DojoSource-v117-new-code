...
function end(mode, type, selection) {
	...
	case 1:
		if (qm.getPlayer().getInventory(Packages.client.inventory.MapleInventoryType.EQUIP).getNumFreeSlot() < 1) {
			...
			qm.getClient().getSession().write(Packages.tools.packet.CWvsContext.getTopMsg("<Tristan's Successor> has been rewarded."));
			qm.getClient().getSession().write(Packages.tools.packet.CWvsContext.serverNotice(5, "<Tristan's Successor> has been rewarded."));
