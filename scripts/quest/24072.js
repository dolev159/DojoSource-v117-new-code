...
function start(mode, type, selection) {
	...
	case 8:
		if (qm.getMap(910150220).getCharacters().size() < 1) {
			...
			qm.getClient().getSession().write(Packages.tools.packet.CWvsContext.serverNotice(5, "This field is already in use. Try again later, or change channels and try again."));
			qm.dispose();
}
}