...
function start(mode, type, selection) {
	...
	case 8:
		if (qm.getMap(921140000).getCharacters().size() < 1) {
			...
			qm.getPlayer().getMap().getMonsterByOid(qm.getPlayer().getMap().getMonsterById(9300295).getObjectId()).switchController(qm.getPlayer(), false);
			qm.getClient().getSession().write(Packages.tools.packet.MobPacket.getNodeProperties(Packages.server.life.MapleLifeFactory.getMonster(9300295), qm.getPlayer().getMap()));
			qm.getClient().getSession().write(Packages.tools.packet.CWvsContext.serverNotice(5, "Escort Ifia to the Audience Room, protecting her from the monsters along the way."));
