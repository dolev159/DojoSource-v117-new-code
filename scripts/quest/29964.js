...
function start(mode, type, selection) {
	...
	case 0:
		...
		if (qm.getPlayer().getQuestNAdd(Packages.server.quest.MapleQuest.getInstance(29964)).getStatus() < 2) {
			qm.getClient().getSession().write(Packages.tools.packet.CWvsContext.getTopMsg("<A Member of The Maple Alliance> has been rewarded."));
			qm.getClient().getSession().write(Packages.tools.packet.CWvsContext.serverNotice(5, "<A Member of The Maple Alliance> has been rewarded."));
