...
function start(mode, type, selection) {
	...
	switch (status) {
	case 0:
		Packages.server.quest.MapleQuest.getInstance(29900).forceStart(qm.getPlayer(), qm.getNpc(), null);
		qm.getClient().getSession().write(Packages.tools.packet.CWvsContext.getShowQuestCompletion(29900));
		qm.getClient().getSession().write(Packages.tools.packet.CWvsContext.serverNotice(5, "You have earned the <Beginner Adventurer> title. You can receive a Medal from NPC Dalair."));
		qm.dispose();
}
}
