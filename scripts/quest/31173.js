...
function action1(mode, type, selection) {
	...
	case 10:
		...
		Packages.server.quest.MapleQuest.getInstance(31184).forceStart(qm.getPlayer(), qm.getNpc(), 0);
		qm.dispose();
}
}
...
function action7(mode, type, selection) {
	...
	case 1:
		if (qm.getPlayer().getSkillLevel(20010194) < 1) {
			qm.getClient().getSession().write(Packages.tools.packet.CWvsContext.serverNotice(5, "You learned the Inherited Will skill."));
