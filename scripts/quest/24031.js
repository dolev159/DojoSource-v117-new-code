...
function end(mode, type, selection) {
	...
	case 0:
		...
		if (qm.getPlayer().getQuestNAdd(Packages.server.quest.MapleQuest.getInstance(24031)).getStatus() < 2) {
			...
			qm.getClient().getSession().write(Packages.tools.packet.CWvsContext.getTopMsg("Sylvidia Riding Skill obtained"));
			qm.getClient().getSession().write(Packages.tools.packet.CWvsContext.serverNotice(5, "You obtained the <Sylvidia Riding> skill."));
			}
