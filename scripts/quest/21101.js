...
function start(mode, type, selection) {
	...
	case 1:
		if (qm.getPlayer().getInventory(Packages.client.inventory.MapleInventoryType.EQUIP).getNumFreeSlot() < 1) {
			...
			qm.getClient().getSession().write(Packages.tools.packet.CWvsContext.getTopMsg("You have acquired the Pig's Weakness skill."));
			qm.getClient().getSession().write(Packages.tools.packet.CWvsContext.serverNotice(5, "You have acquired the Pig's Weakness skill."));
