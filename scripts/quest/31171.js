...
function action5(mode, type, selection) {
	...
	case 3:
		if (qm.getPlayer().getQuestNAdd(Packages.server.quest.MapleQuest.getInstance(31171)).getStatus() < 2) {
			...
			qm.getClient().getSession().write(Packages.tools.packet.CWvsContext.serverNotice(5, "The Polearm has been restored and Maha's power flows again!"));
...
function action6(mode, type, selection) {
	...
	case 7:
		if (qm.getPlayer().getSkillLevel(20000194) < 1) {
			...
			qm.getClient().getSession().write(Packages.tools.packet.CWvsContext.serverNotice(5, "You learned the Regained Memory skill."));
			}
