...
function action(mode, type, selection) {
	...
	case 0:
		...
		if (type == 15 && select == 4) {
		cm.sendNext("Okay, come see me if change your mind.");
		cm.dispose();
		return;
		}
		if (type == 15) {
...
function action4(mode, type, selection) {
	...
	case 1:
		if (cm.getPlayer().itemQuantity(4001160) < 20) {
			cm.sendNext("To make #t1122117#, we need 20 #t4001160#s. You seem to be missing a few.");
			cm.dispose();
			return;
			}
			cm.sendAcceptDecline("I can make you #v1122117# #t1122117# if you have 20 #t4001160#s. Would you like me to make it for you now?");
			break;
	case 2:
...
function action5(mode, type, selection) {
	...
	case 1:
		if (cm.getPlayer().itemQuantity(1122116) < 1 || cm.getPlayer().itemQuantity(1122117) < 1) {
			...
			cm.sendAcceptDecline("Oh, you've brought #b#t1122117##k! How did you know it was mine? And you have #b#t1122116##k, as well. I can combine these two pendants to create the #v1122118# #b#t1122118##k. Shall we do that?");
			break;
	case 2:
		if (cm.getPlayer().getInventory(Packages.client.inventory.MapleInventoryType.EQUIP).getNumFreeSlot() < 1) {
			cm.sendNext("I don't think you have any empty slots in your Inventory. Please check again.");
