...
function setup(level, leaderid) {//開始事件，時間
	...
	eim.setInstanceMap(926110203).spawnNpc(2112010, new java.awt.Point(240, 243));
	eim.setInstanceMap(926110401).spawnNpc(2112010, new java.awt.Point(200, 100));

	eim.setInstanceMap(926110600).spawnNpc(2112005, new java.awt.Point(290, 100));
	eim.setInstanceMap(926110600).spawnNpc(2112006, new java.awt.Point(340, 100));
...
function playerEntry(eim, player) {//傳送進事件地圖
	...
	player.getClient().getSession().write(Packages.tools.packet.CField.NPCPacket.getNPCTalk(2112003, 0, "This is the lab where rumors are abound that a suspicious noise can be heard from here every night. If there's anything hidden in here, it has to be in this place. Please look thoroughly into this lab.", "00 00", 0));
}

function monsterValue(eim, player, mob) {//殺怪後觸發
	if (mob.getMap().getId() == 926110001 && mob.getMap().getAllMonstersThreadsafe().size() < 1) {
	...
	if (mob.getId() == 9300151 || mob.getId() == 9300152) {
		...
		eim.getMapInstance(926110401).killMonster(9300138);
		eim.getMapInstance(926110401).killMonster(9300150);
		eim.getMapInstance(926110401).killMonster(9300150);

		eim.getMapInstance(926110401).setSpawns(false);//限制刷怪

		eim.getMapInstance(926110401).spawnNpc(2112006, new java.awt.Point(-20, 100));
		eim.getMapInstance(926110401).spawnNpc(2112005, new java.awt.Point(40, 100));
		...
		eim.getMapInstance(926110401).broadcastMessage(Packages.tools.packet.CWvsContext.serverNotice(6, "Successfully saved Romeo."));
