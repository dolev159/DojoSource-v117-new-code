...
function enter(pi) {
	if (pi.getPlayer().getMap().containsNPC(2112000)) {
		pi.removeNpc(pi.getPlayer().getMap().getId(), 2112000);
		for (var i = 0; i < pi.getPlayer().getMap().getCharactersThreadsafe().size(); i++)
		pi.getPlayer().getMap().getCharacterById(pi.getPlayer().getMap().getCharactersThreadsafe().get(i).getId()).getClient().getSession().write(Packages.tools.packet.CField.NPCPacket.getNPCTalk(2112000, 0, "Who is it that's barging into my lab without permission!! My lab report is not for any one of you!!!", "00 00", 0));
		...
		pi.getPlayer().getMap().startMapEffect("Please defeat all the monsters!", 5120021);
